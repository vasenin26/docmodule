...
<template>
    <div class="rounded-lg p-3 bg-blue-50 border border-blue-200">
        ...
        <div class="text-sm">
            <div v-if="!message.message.content" class="text-blue-500 italic">Информационное сообщение без текстового содержимого</div>
            <div v-else-if="isLongMessage(message.message.content) && !expandedMessages.has(props.index)" class="space-y-2">
                <div class="overflow-x-hidden break-words whitespace-pre-wrap text-blue-800">{{ getTruncatedContent(message.message.content) }}</div>
                <button @click="toggleMessageExpansion(props.index)" class="text-xs font-medium text-blue-600 hover:text-blue-800">
                    Показать полностью
                </button>
            </div>
            <div v-else class="space-y-2">
                <div class="overflow-x-hidden break-words whitespace-pre-wrap text-blue-800">{{ message.message.content }}</div>
                <button
