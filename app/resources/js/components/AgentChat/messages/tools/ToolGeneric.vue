...
function getToolName(): string {
    return props.message.message.name || (props.message.message as any).tool_name || getFunctionName((props.message.message as any).tool_call_id || '') || 'Неизвестный инструмент';
}

function getToolArgs(): string | undefined {
    return props.message.message.args || (props.message.message as any).tool_args;
}

function getToolResult(): string | undefined {
    return props.message.message.result || (props.message.message as any).tool_result;
}
        ...
        <div class="text-sm space-y-3">
            <div v-if="getToolArgs()" class="space-y-1">
                ...
                <div v-if="isLongText(getToolArgs(), 100) && !isTextExpanded('tool-args')" class="space-y-1">
                    <div class="text-sm font-mono bg-gray-100 p-2 rounded border overflow-x-auto">{{ getTruncatedText(getToolArgs(), 100) }}</div>
                    <button @click="toggleTextExpansion('tool-args')" class="text-xs font-medium text-blue-600 hover:text-blue-800">Показать полностью</button>
                </div>
                <div v-else class="space-y-1">
                    <div class="text-sm font-mono bg-gray-100 p-2 rounded border overflow-x-auto">{{ getToolArgs() }}</div>
                    <button v-if="isLongText(getToolArgs(), 100)" @click="toggleTextExpansion('tool-args')" class="text-xs font-medium text-blue-600 hover:text-blue-800">Свернуть</button>
                </div>
            </div>
            ...
            <div v-if="getToolResult()" class="space-y-1">
                <div class="flex items-center justify-between">
                    ...
                    <button @click="toggleTextExpansion('tool-result')" class="text-xs font-medium text-blue-600 hover:text-blue-800">
            ...
            <div v-if="props.message.message.content">
                ...
                <div v-if="isLongMessage(props.message.message.content) && !expandedMessages.has(props.index)" class="space-y-2">
                    ...
                    <button @click="toggleMessageExpansion(props.index)" class="text-xs font-medium text-blue-600 hover:text-blue-800">Показать полностью</button>
                </div>
                <div v-else class="space-y-2">
                    <div class="overflow-x-hidden break-words whitespace-pre-wrap">{{ props.message.message.content }}</div>
                    <button v-if="isLongMessage(props.message.message.content)" @click="toggleMessageExpansion(props.index)" class="text-xs font-medium text-blue-600 hover:text-blue-800">Свернуть</button>
                </div>
            </div>

            <div v-if="!props.message.message.content && !getToolName() && !getToolArgs() && !getToolResult()" class="text-gray-500 italic">Сообщение без содержимого</div>
        </div>
    </div>
</template>
