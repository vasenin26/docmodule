...
function parseResult(): { success?: boolean; message?: string; data?: any } | undefined {
    ...
    try {
        ...
        if (parsed && parsed.payload) return parsed.payload as any;
        ...
        <div class="text-sm space-y-3">
            <div class="space-y-1">
                ...
                <div class="text-sm bg-white p-2 rounded border overflow-x-auto whitespace-pre-wrap">
                    {{ (typeof parseResult() === 'object' && (parseResult() as any).message) ? (parseResult() as any).message : (getSuccess() ? 'Анализ успешно выполнен' : 'Анализ завершился с ошибкой') }}
                </div>
            </div>

            <template v-if="parseResult()">
                <div v-if="(parseResult() as any)?.project_type" class="space-y-1">
                    <div class="text-xs font-medium text-gray-600">Тип проекта:</div>
                    <div class="text-sm bg-white p-2 rounded border overflow-x-auto">{{ (parseResult() as any)?.project_type }}</div>
                </div>

                <div v-if="Array.isArray((parseResult() as any)?.main_directories)" class="space-y-1">
                    <div class="text-xs font-medium text-gray-600">Основные директории ({{ parseResult()?.data?.main_directories.length }}):</div>
                    <ul class="list-disc pl-5 space-y-1">
                        <li v-for="dir in (parseResult() as any)?.main_directories" :key="dir" class="bg-white p-2 rounded border">{{ dir }}</li>
                    </ul>
                </div>

                <div v-if="Array.isArray((parseResult() as any)?.entry_points)" class="space-y-1">
                    <div class="text-xs font-medium text-gray-600">Точки входа ({{ parseResult()?.data?.entry_points.length }}):</div>
                    <ul v-if="(parseResult() as any)?.entry_points.length" class="list-disc pl-5 space-y-1">
                        <li v-for="ep in (parseResult() as any)?.entry_points" :key="ep" class="bg-white p-2 rounded border">{{ ep }}</li>
                    </ul>
                    <div v-else class="text-gray-500 italic">не найдены</div>
                </div>

                <div v-if="Array.isArray((parseResult() as any)?.config_files)" class="space-y-1">
                    <div class="text-xs font-medium text-gray-600">Конфигурационные файлы ({{ parseResult()?.data?.config_files.length }}):</div>
                    <ul v-if="(parseResult() as any)?.config_files.length" class="list-disc pl-5 space-y-1">
                        <li v-for="cf in (parseResult() as any)?.config_files" :key="cf" class="bg-white p-2 rounded border">{{ cf }}</li>
                    </ul>
                    <div v-else class="text-gray-500 italic">не найдены</div>
                </div>
            </template>
