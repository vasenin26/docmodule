        ...
        <template v-if="parseResult()?.data">
            <div class="text-sm space-y-3">
                <div class="space-y-1">
                    ...
                    <div class="bg-white p-2 rounded border text-xs font-mono overflow-x-auto">{{ parseResult()?.data?.pattern }}</div>
                    <div class="text-xs text-gray-600">режим: {{ parseResult()?.data?.mode }}; модификаторы: {{ parseResult()?.data?.modifiers }}</div>
                </div>

                <div class="grid grid-cols-2 gap-2 text-xs">
                    <div class="bg-white p-2 rounded border">совпадений: {{ parseResult()?.data?.matches_count }}</div>
                    <div class="bg-white p-2 rounded border">расширения: {{ (parseResult()?.data?.file_extensions || []).join(', ') }}</div>
                </div>
                ...
                <div class="space-y-1">
                    ...
                    <ul v-if="showMatches && Array.isArray(parseResult()?.data?.matches) && parseResult()?.data?.matches.length" class="space-y-1">
