...
<template>
    <div class="rounded-lg p-3" :class="containerClass()">
        <ToolHeaderStatus :title="'добавление файла в список'" :isError="!getSuccess()" />

        <template v-if="parseResult()">
            <div class="text-sm space-y-3">
                <div class="space-y-1">
                    <div class="text-xs font-medium text-gray-600">Файл:</div>
                    <div class="text-sm bg-white p-2 rounded border font-mono overflow-x-auto">{{ parseResult()?.file_path || parseResult()?.filename }}</div>
                </div>

                <div class="space-y-1">
                    <div class="text-xs font-medium text-gray-600">Список:</div>
                    <div class="text-sm bg-white p-2 rounded border">{{ parseResult()?.list_name || parseResult()?.list || 'Не указан' }}</div>
                </div>

                <div class="grid grid-cols-2 gap-2 text-xs">
                    <div class="bg-white p-2 rounded border">
                        <div class="font-medium text-gray-600">Позиция:</div>
                        <div class="text-sm">{{ parseResult()?.position || parseResult()?.index || 'В конец' }}</div>
                    </div>
                    <div class="bg-white p-2 rounded border">
                ...
                <div v-if="parseResult()?.list_size" class="space-y-1">
                    <div class="text-xs font-medium text-gray-600">Размер списка:</div>
                    <div class="text-sm bg-white p-2 rounded border">{{ parseResult()?.list_size }} элементов</div>
                </div>
            </div>
        </template>
