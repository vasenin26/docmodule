<?php

namespace Tests\Feature;

use App\Jobs\CalculateVersionDifferenceJob;
use App\Models\Page;
use App\Models\User;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Illuminate\Support\Facades\Queue;
use Tests\TestCase;

class PageControllerTest extends TestCase
{
    use RefreshDatabase;

    public function test_approve_draft()
    {
        Queue::fake();

        $user = User::factory()->create();
        $page = Page::factory()->withVersions()->create(['created_by' => $user->id]);
        // Ensure the page has a current version before creating a draft
        $this->actingAs($user);
        $this->withoutMiddleware();
        $this->post(route('pages.store'), [
            'title' => 'Initial',
            'content' => '<p>Content</p>',
            'parent_id' => null,
            'project_id' => null,
        ]);
        // Create draft from current version via controller method to maintain consistency
        $draft = $page->createDraft(['title' => 'Draft Title']);

        $response = $this->post(route('drafts.approve', $draft));

        $response->assertRedirect();
    }

    public function test_approve_draft_dispatches_task_creation_job()
    {
        Queue::fake();

        $user = User::factory()->create();
        $page = Page::factory()->withVersions()->create(['created_by' => $user->id]);
        // Ensure the page has a current version before creating a draft
        $this->actingAs($user);

        $this->post(route('pages.store'), [
            'title' => 'Initial',
            'content' => '<p>Content</p>',
            'parent_id' => null,
            'project_id' => null,
        ]);
        // Create draft from current version via controller method to maintain consistency
        $draft = $page->createDraft(['title' => 'Draft Title']);

        $response = $this->post(route('drafts.approve', $draft), [
            'title' => 'New title',
            'createTask' => true,
        ]);

        $response->assertSessionDoesntHaveErrors();
        $response->assertRedirect();

        $previousVersionId = $draft->previous_version_id;
//        Queue::assertPushed(CalculateVersionDifferenceJob::class, function ($job) use ($draft, $previousVersionId) {
//            return $job->newVersionId === $draft->id && $job->oldVersionId === $previousVersionId;
//        });
    }

    public function test_approve_draft_validates_required_fields()
    {
        $user = User::factory()->create();
        $page = Page::factory()->withVersions()->create(['created_by' => $user->id]);
        $this->actingAs($user);
        $this->withoutMiddleware();

        // Create a draft
        $draft = $page->createDraft(['title' => 'Draft Title']);

        // Test without required 'title' field - валидация НЕ пройдет, контроллер НЕ выполнится
        $response = $this->post(route('drafts.approve', $draft->id), [
            'content' => '<p>Some content</p>',
            'createTask' => true,
        ]);

        // Проверяем, что произошел редирект с ошибками валидации
        $response->assertRedirect();
        $response->assertSessionHasErrors(['title']);

        // Проверяем, что контроллер НЕ выполнился (нет var_dump в выводе)
        $this->assertStringNotContainsString('Controller reached!', $response->getContent());
    }

    public function test_approve_draft_with_valid_data_reaches_controller()
    {
        $user = User::factory()->create();
        $page = Page::factory()->withVersions()->create(['created_by' => $user->id]);
        $this->actingAs($user);
        $this->withoutMiddleware();

        // Create a draft
        $draft = $page->createDraft(['title' => 'Draft Title']);

        // Test with valid data - валидация пройдет, контроллер выполнится
        $response = $this->post(route('drafts.approve', $draft), [
            'title' => 'Valid Title',
            'content' => '<p>Some content</p>',
            'createTask' => true,
        ]);

        // Проверяем, что контроллер выполнился (есть var_dump в выводе)
        $this->assertStringContainsString('Controller reached!', $response->getContent());
    }

    public function test_approve_draft_validates_title_max_length()
    {
        $user = User::factory()->create();
        $page = Page::factory()->withVersions()->create(['created_by' => $user->id]);
        $this->actingAs($user);
        $this->withoutMiddleware();

        // Create a draft
        $draft = $page->createDraft(['title' => 'Draft Title']);

        // Test with title longer than 255 characters
        $longTitle = str_repeat('a', 256);
        $response = $this->post(route('drafts.approve', $draft->id), [
            'title' => $longTitle,
            'content' => '<p>Some content</p>',
            'createTask' => true,
        ]);

        // Проверяем, что произошел редирект с ошибками валидации
        $response->assertRedirect();
        $response->assertSessionHasErrors(['title']);
    }

    public function test_approve_draft_validates_project_files_structure()
    {
        $user = User::factory()->create();
        $page = Page::factory()->withVersions()->create(['created_by' => $user->id]);
        $this->actingAs($user);
        $this->withoutMiddleware();

        // Create a draft
        $draft = $page->createDraft(['title' => 'Draft Title']);

        // Test with invalid project_files structure
        $response = $this->post(route('drafts.approve', $draft->id), [
            'title' => 'Valid Title',
            'content' => '<p>Some content</p>',
            'project_files' => [
                ['url' => 'not-a-valid-url', 'description' => 'Test']
            ],
            'createTask' => true,
        ]);

        // Проверяем, что произошел редирект с ошибками валидации
        $response->assertRedirect();
        $response->assertSessionHasErrors(['project_files.0.url']);
    }

    public function test_approve_draft_does_not_dispatch_job_for_non_draft()
    {
        Queue::fake();

        $user = User::factory()->create();
        $page = Page::factory()->create(['created_by' => $user->id]);
        $this->actingAs($user);
        $this->withoutMiddleware();

        $response = $this->post(route('drafts.approve', $page->id));

        $response->assertRedirect();

        // Проверяем, что Job НЕ был запущен для не-черновика
        Queue::assertNotPushed(CalculateVersionDifferenceJob::class);
    }
}
